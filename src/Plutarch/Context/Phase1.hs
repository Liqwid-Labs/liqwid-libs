...
type Checker = forall a. Builder a => a -> Validation [String] a

{- | Checks if inputs, outputs, and minting is correct and zero sum.

 @since 1.2.0
-}
inOutZeroSum :: Checker
inOutZeroSum b@(unpack -> BB{..})
    | i <> m == o <> bbFee = Success b
    | otherwise =
        Failure
            [ "Inputs, Outputs, and Mints are not zero sum. Diff: "
